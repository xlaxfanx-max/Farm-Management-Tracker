  ...
  return (
    <div className={`flex items-start gap-3 py-3 border-b border-gray-100 last:border-0`}>
      ...
      <div className="flex-1 min-w-0">
        ...
        {item.detail && (
          <p className={`text-xs mt-0.5 ${item.is_passed ? 'text-gray-500' : 'text-red-600'}`}>
            {item.detail}
          </p>
        )}
...
function SectionCard({ section, onNavigate }) {
  ...
  return (
    <div className={`bg-white rounded-xl border shadow-sm overflow-hidden ${
      allGood ? 'border-green-200' : 'border-gray-200'
    }`}>
      <button
        onClick={() => setOpen((v) => !v)}
      ...
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-4xl mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900">Inspector Readiness</h1>
              <p className="text-sm text-gray-500">Review what an inspector would look for</p>
            </div>
          </div>
