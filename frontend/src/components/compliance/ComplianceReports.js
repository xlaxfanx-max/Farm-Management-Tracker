...
const ReportTypeBadge = ({ type }) => {
  const styles = {
    pur_monthly: 'bg-sand-200 text-bark-700',
    sgma_semi_annual: 'bg-orange-100 text-orange-700',
    ilrp_annual: 'bg-green-100 text-green-700',
    wps_annual: 'bg-green-100 text-primary',
    custom: 'bg-cream-100 text-bark-700',
  };
...
const StatusBadge = ({ status }) => {
  const styles = {
    draft: 'bg-cream-100 text-bark-700',
...
const ReportCard = ({ report, onView, onDownload, onSubmit, onDelete }) => {
  ...
  const getStatusIcon = () => {
    switch (report.status) {
      ...
      case 'ready':
        return <FileCheck className="w-5 h-5 text-link" />;
      case 'pending_review':
        return <Clock className="w-5 h-5 text-yellow-600" />;
      case 'rejected':
        return <AlertTriangle className="w-5 h-5 text-danger" />;
      default:
        return <FileText className="w-5 h-5 text-bark-600" />;
    }
  };
        ...
        <div className="flex items-center gap-3">
          <div className={`w-10 h-10 rounded-lg flex items-center justify-center
            ${report.status === 'accepted' || report.status === 'submitted' ? 'bg-green-100' :
              report.status === 'ready' ? 'bg-orange-100' :
              report.status === 'pending_review' ? 'bg-yellow-200' :
              report.status === 'rejected' ? 'bg-danger-bg' : 'bg-cream-100'}`}>
            {getStatusIcon()}
          </div>
          <div>
            ...
            <div>
              ...
              <div className="bg-cream-50 rounded-lg p-4 space-y-2 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className="text-heading">{formatDate(report.submitted_at)}</span>
                </div>
                {report.submitted_by_name && (
                  <div className="flex justify-between">
                    <span className="text-bark-600">By:</span>
                    <span className="text-heading">{report.submitted_by_name}</span>
                  </div>
                )}
                {report.submission_reference && (
                  <div className="flex justify-between">
                    <span className="text-bark-600">Reference:</span>
                    <span className="text-heading">{report.submission_reference}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                {report.validation_warnings?.map((warning, idx) => (
                  <div key={idx} className="flex items-start gap-2 text-yellow-600 text-sm bg-yellow-100 rounded-lg p-3">
                    <AlertTriangle className="w-4 h-4 mt-0.5 flex-shrink-0" />
                    <span>{warning}</span>
                  </div>
                ))}
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-border">
            {report.report_file && (
              <a
                ...
                className="px-4 py-2 text-bark-700 hover:bg-cream-100 rounded-lg transition-colors flex items-center gap-2"
              >
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={`relative bg-surface-raised rounded-card shadow-xl w-full ${maxWidth} overflow-y-auto max-h-[90vh]`}>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            <label className="block text-sm font-medium text-bark-700 mb-1">Report Type *</label>
            <select
              required
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-bark-700 mb-1">Period Start *</label>
              <input
                type="date"
                required
            ...
            <div>
              <label className="block text-sm font-medium text-bark-700 mb-1">Period End *</label>
              <input
                type="date"
                required
