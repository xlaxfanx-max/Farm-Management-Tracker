...
const WellModal = ({ isOpen, onClose, well, waterSources, onSave }) => {
  ...
  const footer = (
    <>
      ...
      <button
        ...
        form="well-form"
        disabled={loading}
        className="flex items-center gap-2 px-4 py-2 rounded-button bg-primary text-white hover:bg-primary-hover disabled:opacity-50"
      >
  ...
  return (
    <Modal
      ...
      footer={footer}
    >
      <div className="flex border-b border-border -mx-6 -mt-4 mb-4 px-6" role="tablist">
        {tabs.map((tab) => {
          const Icon = tab.icon;
            ...
            <div className="space-y-4">
              <label className="flex items-center gap-2">
                <input
                  ...
                  className="rounded border-border-strong text-link focus:ring-ring"
                />
                <span className="text-sm text-bark-700">Registered with GSA</span>
              </label>
