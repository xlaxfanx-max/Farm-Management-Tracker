    ...
    <div className="space-y-5">
      ...
      {hasAnyData ? (
          ...
          {cards.map((card) => (
            ...
            >
              <div className="flex items-center justify-between mb-3">
                <div>
                  <h3 className="font-semibold text-gray-900">{card.commodity}</h3>
                  <p className="text-xs text-gray-500">Season {card.season_label}</p>
                </div>
              ...
              <div className="space-y-1 text-sm">
                {card.net_return != null && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Settlement net</span>
                    <span className="font-medium text-gray-900">{formatCurrency(card.net_return)}</span>
                  </div>
                )}
                {card.pickhaul_cost != null && Number(card.pickhaul_cost) > 0 && (
                  <div className="flex justify-between">
                    <span className="text-gray-500">Pick &amp; haul</span>
                    <span className="font-medium text-gray-900">−{formatCurrency(card.pickhaul_cost)}</span>
                  </div>
                )}
                {card.net_to_grower != null && (
                  <div className="flex justify-between border-t border-gray-100 pt-1">
                    <span className="text-gray-600 font-medium">Net to grower</span>
              ...
              {(card.committed || (card.actual_houses || []).length > 0) && (
                <div className="mt-3 pt-2 border-t border-gray-100 flex flex-wrap items-center gap-1.5 text-xs">
                  ...
                  {(card.actual_houses || []).map((h) => (
                    <span key={h} className="px-1.5 py-0.5 bg-gray-100 text-gray-600 rounded">
                      {h}
                    </span>
                  ))}
                </div>
        ...
        <div className="text-center py-10 bg-gray-50 rounded-xl">
          ...
          <p className="text-gray-600 font-medium">No season data yet</p>
